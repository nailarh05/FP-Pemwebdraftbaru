...
export default function PlayUnjumble() {
    ...
    const validateAnswer = useCallback(() => {
        ...
        const normalizedTarget = currentSentence.sentence_text.trim().replace(/\s+/g, ' ');

        const isCorrect = normalizedPlayer === normalizedTarget;

        if (!isCorrect) {
            console.log(`Validation Failed: Expected '[${normalizedTarget}]', Got '[${normalizedPlayer}]'`);
        }
            ...
            <header className="flex justify-between items-center p-4 bg-white shadow-md rounded-lg mb-6">
                <div>
                    ...
                    <p className="text-sm text-indigo-600 mt-1 italic">
                        Sentence {currentSentenceIndex + 1} of {game.sentences.length} — Arrange the words correctly
                    </p>
                </div>
