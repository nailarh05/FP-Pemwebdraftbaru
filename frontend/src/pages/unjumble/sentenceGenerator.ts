...
export const generateSentences = async (
    ...
    if (normalizedTopic.includes("nature") || normalizedTopic.includes("animal")) {
        pool = SENTENCE_TEMPLATES["nature"];
    } else if (normalizedTopic.includes("grammar") || normalizedTopic.includes("english")) {
        pool = SENTENCE_TEMPLATES["grammar"];
    } else if (normalizedTopic.includes("code") || normalizedTopic.includes("programm")) {
        pool = SENTENCE_TEMPLATES["coding"];
    } else if (normalizedTopic.includes("history") || normalizedTopic.includes("past")) {
        pool = SENTENCE_TEMPLATES["history"];
    } else if (normalizedTopic.includes("logic") || normalizedTopic.includes("logika")) {
    ...
    const knownCategories = ["nature", "animal", "grammar", "english", "code", "programm", "history", "past", "logic", "logika", "general"];
    const isUnknown = !knownCategories.some(cat => normalizedTopic.includes(cat));
